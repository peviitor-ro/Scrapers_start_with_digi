...
def update_molTaleo():
    ...
    clean_request = requests.post(CLEAN_URL, headers=clean_header, data={'company': 'MOL'})
    ...
    with open('scrapers_forzza/data_mol.json', 'r') as file:
        data = json.load(file)

    post_request_to_server = requests.post('https://api.peviitor.ro/v4/update/', headers=post_header, data=json.dumps(data))
    print(f'Mol post data on server response = {post_request_to_server}')
